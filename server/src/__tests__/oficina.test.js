...
describe("Oficina", () => {
  ...
  beforeEach(async () => {
    ...
    coordenadorUser = await createTestUser({
    ...
    visitanteUser = await createTestUser({
      nome: "Visitante User",
    ...
    coordenadorToken = getAuthToken(coordenadorUser);
    visitanteToken = getAuthToken(visitanteUser);
  });
  ...
  describe("POST /oficinas - Criar oficina", () => {
    ...
    it("deve criar uma oficina com sucesso (coordenador)", async () => {
      const oficinaData = {
        titulo: "Oficina do Coordenador",
        descricao: "Descrição",
      };
    ...
    it("não deve criar oficina sem autenticação", async () => {
      ...
      await request(app).post("/oficinas").send(oficinaData).expect(401);
    });

    it("não deve criar oficina sem autorização (visitante role)", async () => {
      const oficinaData = {
        titulo: "Oficina Teste",
        descricao: "Descrição teste",
      };
  ...
  describe("GET /oficinas - Listar oficinas", () => {
    ...
    it("deve listar oficinas para visitante (leitura permitida)", async () => {
      const o1 = new Oficina({
        titulo: "Oficina 1",
      });
      await o1.save();
    ...
    it("não deve listar oficinas sem autenticação", async () => {
      await request(app).get("/oficinas").expect(401);
    });
  ...
  describe("GET /oficinas/:id - Obter oficina por ID", () => {
    ...
    it("não deve obter oficina sem autenticação", async () => {
      ...
      await request(app).get(`/oficinas/${oficina._id}`).expect(401);
    });
  });
  ...
  describe("PUT /oficinas/:id - Atualizar oficina", () => {
    ...
    it("deve atualizar uma oficina com sucesso (coordenador)", async () => {
      const oficina = new Oficina({
        titulo: "Oficina Original",
      });
      await oficina.save();
      ...
      const response = await request(app)
        .put(`/oficinas/${oficina._id}`)
        .set("Authorization", `Bearer ${coordenadorToken}`)
        .send({ titulo: "Atualizada pelo Coordenador" })
        .expect(200);

      expect(response.body.titulo).toBe("Atualizada pelo Coordenador");
    });
    ...
    it("não deve atualizar sem autorização (visitante role)", async () => {
      const oficina = new Oficina({
        titulo: "Oficina Teste",
      });
      await oficina.save();
  ...
  describe("DELETE /oficinas/:id - Deletar oficina", () => {
    ...
    it("não deve deletar sem autenticação", async () => {
      ...
      await request(app).delete(`/oficinas/${oficina._id}`).expect(401);
    });

    it("não deve deletar sem autorização (coordenador role)", async () => {
      const oficina = new Oficina({
        titulo: "Oficina Teste",
      });
      await oficina.save();
    ...
    it("não deve deletar sem autorização (visitante role)", async () => {
      const oficina = new Oficina({
        titulo: "Oficina Teste",
      });
      await oficina.save();
