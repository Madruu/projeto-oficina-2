...
describe("Voluntário CRUD", () => {
  ...
  describe("POST /voluntarios - Criar voluntário", () => {
    ...
    it("deve criar um voluntário com sucesso (coordenador)", async () => {
      const voluntarioData = {
    ...
    it("não deve criar voluntário sem autenticação", async () => {
      ...
      await request(app).post("/voluntarios").send(voluntarioData).expect(401);
    });

    it("não deve criar voluntário sem autorização (visitante role)", async () => {
      const voluntarioData = {
        nomeCompleto: "João Silva",
        cpf: "12345678900",
      };
  ...
  describe("GET /voluntarios - Listar voluntários", () => {
    ...
    it("deve listar todos os voluntários (visitante - leitura permitida)", async () => {
      const v1 = new Voluntario({
        nomeCompleto: "Voluntário 1",
        cpf: "11111111111",
      });
    ...
    it("não deve listar voluntários sem autenticação", async () => {
      await request(app).get("/voluntarios").expect(401);
    });
  });
  ...
  describe("PUT /voluntarios/:id - Atualizar voluntário", () => {
    ...
    it("deve atualizar um voluntário com sucesso (coordenador)", async () => {
      const voluntario = new Voluntario({
        nomeCompleto: "Voluntário Original",
        cpf: "12345678900",
      });
      ...
      const response = await request(app)
        .put(`/voluntarios/${voluntario._id}`)
        .set("Authorization", `Bearer ${coordenadorToken}`)
        .send({ nomeCompleto: "Atualizado pelo Coordenador" })
        .expect(200);

      expect(response.body.nomeCompleto).toBe("Atualizado pelo Coordenador");
    });
  ...
  describe("DELETE /voluntarios/:id - Deletar voluntário", () => {
    ...
    it("não deve deletar sem autenticação", async () => {
      ...
      await request(app).delete(`/voluntarios/${voluntario._id}`).expect(401);
    });
